...
class AIService {
  // In a real app, this should be fetched securely or via environment variables
  static const String apiKey = ''; // User needs to provide their key

  late final GenerativeModel _model;

  AIService({String? customKey}) {
    final key = customKey ?? apiKey;
  ...
  Future<List<Map<String, dynamic>>> generateCropRecommendations({
    ...
    final prompt = 'Act as an agricultural expert. Suggest 4 crops suitable for farming in $location during $season season with $soilType soil. '
  ...
  Future<Map<String, dynamic>> analyzePriceTrends({
    ...
    final prompt = 'Act as an agricultural economist. Analyze price trends for $crop over the next $period. '
  ...
  Future<String> smartSearch(String query) async {
    final prompt = 'Act as a smart search assistant for an agricultural marketplace. '
